...
export default async function loader() {
  ...
  return session?.user.role === UserRole.USER ? (
    ...
    <div>
      <div className="grid grid-cols-1 gap-1 md:grid-cols-2 md:gap-6 lg:grid-cols-3">
        {Array.from({ length: 6 }).map((_, index) => (
          <Card
            key={index}
            className="w-full overflow-hidden rounded-xl border border-neutral-200 bg-neutral-300 text-neutral-800 shadow-md backdrop-blur-md transition-all duration-300 ease-in-out dark:border-neutral-800/50 dark:bg-neutral-900 dark:text-neutral-200 dark:shadow-lg"
          >
